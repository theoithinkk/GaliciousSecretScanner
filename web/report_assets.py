...
report_assets.py
----------------
The CSS and JS for the HTML report, split out of reporters.py so that file
stays about rendering structure rather than being 90% stylesheet.
...
REPORT_CSS / REPORT_JS cover the page itself: severity tiles, the findings
list, sort and filter, and the back-to-form link.

The finding-detail terminal panel lives in terminal_assets.py. Both files'
CSS is concatenated into one <style> block by reporters.py, so a rule may sit
in either file -- the split is about keeping each under 500 lines, not about
cascade scoping. The shared prefers-reduced-motion block is in
terminal_assets.py and covers elements from both.
"""
...
# The terminal detail panel. The open animation is a CRT/glitch effect: the
# window arrives as horizontal slices (clip-path insets jumping around) with an
# RGB split, then settles. Everything collapses to a plain fade when the user
...
# The detail panel: opens on a finding click, types out the finding, and (when
# served by web/app.py rather than opened from disk) offers the one-click fix.
